...
const Pasien = () => {
  ...
  // Filter dan urutkan pasien: semua nama berawalan 'A' di atas
  const filteredPatients = patients
    ...
    .sort((a, b) => {
      const aIsA = a.name.trim().toLowerCase().startsWith('a');
      const bIsA = b.name.trim().toLowerCase().startsWith('a');
        ...
        <div className="patients-cards-list">
          {filteredPatients.length === 0 ? (
            ...
            filteredPatients.map((patient) => (
              <div key={patient.id} className="hero-patient-a patient-card-horizontal">
                <div className="hero-patient-content">
                  <div className="hero-avatar">{patient.avatar}</div>
                  <div className="hero-info">
                    <div className="hero-name">{patient.name}</div>
                    <div className="hero-id">ID: {patient.id} &nbsp;|&nbsp; {patient.age} tahun &nbsp;|&nbsp; {patient.gender}</div>
                    <div className="hero-diagnosis">{patient.diagnosis}</div>
                    <span className={`status-badge ${getStatusClass(patient.status)}`}>{patient.status}</span>
                  </div>
                  <div className="hero-actions">
                    <button className="btn-action view" title="Lihat Detail" onClick={() => handleViewPatient(patient)}>
                    ...
                    <button className="btn-action edit" title="Edit" onClick={() => handleEditPatient(patient)}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>
                    </button>
                    {patient.status === 'Pulang' ? (
                      <button className="btn-action report" title="Lihat Laporan" onClick={() => handleViewReport(patient)}>
                      ...
                      <button className="btn-action askep" title="Buat Askep" onClick={() => handleCreateAskep(patient)}>
