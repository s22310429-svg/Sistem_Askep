...
const Regis = ({ onRegister }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setError('');
    onRegister && onRegister({ username: form.username, email: form.email, fullname: form.fullname, role: 'user' });
  };
      ...
      <div className="regis-card">
        <div className="regis-left">
          ...
          <ul className="regis-benefit-list">
            <li>✔️ Akses dashboard asuhan keperawatan yang komprehensif.</li>
            <li>✔️ Simpan dan pantau data pasien serta hasil analisis.</li>
            <li>✔️ Dapatkan insight terbaru untuk perencanaan dan evaluasi asuhan.</li>
          </ul>
          <div className="regis-note">
            <span>🛡️ Data Anda kami lindungi dengan standar keamanan terbaru.</span>
          </div>
        </div>
          ...
          <form className="regis-form" onSubmit={handleSubmit} autoComplete="off">
            ...
            <div className="input-group icon-input">
              ...
              <div className="password-input-wrapper">
                ...
                <button type="button" className="show-password-btn" onClick={() => setShowPassword((v) => !v)} tabIndex={-1} aria-label="Tampilkan Password">{showPassword ? '🙈' : '👁️'}</button>
              </div>
            </div>
            <div className="input-group icon-input">
              ...
              <div className="password-input-wrapper">
                ...
                <button type="button" className="show-password-btn" onClick={() => setShowConfirm((v) => !v)} tabIndex={-1} aria-label="Tampilkan Password">{showConfirm ? '🙈' : '👁️'}</button>
              </div>
            </div>
            <div className="checkbox-group">
              <input id="agree" type="checkbox" checked={form.agree} onChange={handleChange} />
              <label htmlFor="agree">Saya setuju dengan <a href="#">syarat dan ketentuan</a> penggunaan sistem ini.</label>
            </div>
            <button type="submit" className="btn btn-login">Daftar</button>
          </form>
